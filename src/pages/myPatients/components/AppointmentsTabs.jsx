...
export default function AppointmentsTabs() {
  ...
  React.useEffect(() => {
    ...
    const fetchAppointments = async () => {
      try {
        const response = await fetch(import.meta.env.VITE_API_URL +"/appointments/doctor/me", {
          method: "GET",
          headers: {
  ...
  const pendingDiagnosisAppointments = pastAppointments.filter(app => app.diagnosis === "");
      ...
      >
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          ...
          >
            ...
            <Tab label="Citas pendientes de diagnostico" {...a11yProps(1)} />
  ...
  <Typography variant="body1" sx={{ ml: 3 }}>
    No hay citas pasadas
        ...
        <CustomTabPanel value={value} index={1}>
          <Grid container spacing={2}>
          {pendingDiagnosisAppointments.length > 0 ? (
  pendingDiagnosisAppointments.map((appointment) => (
    <Grid item xs={12} sm={12} md={6} key={appointment.id}>
      <AppointmentCard
  ...
  <Typography variant="body1" sx={{ ml: 3 }}>
    No hay citas pendientes de diagnóstico
  </Typography>
)}
